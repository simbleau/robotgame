...
def tentative_act(this_robot, game, illegals):
    ...
    if (verbose): print
    "current location: ", this_robot.location
    if (verbose > 1): print
    "bots 1 away: ", one_robots
    if (verbose > 1): print
    "bots 2 away: ", two_robots
...
def act_with_consideration(this_robot, game, illegals):
    ...
    for bot in sorted_bots:
        if (verbose > 1): print
        "looking at location ", bot.location, ", urgency ", urgency(bot, game)
        move = act_with_illegals(bot, game, illegals)
        square = destination_square(bot, move)
        if (verbose > 1): print
        "move: ", move, " square taken: ", square
        illegals.add(square)
        if (bot == this_robot):
            if (verbose > 1): print
            illegals
